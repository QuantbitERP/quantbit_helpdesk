...
def book_expense(**kwargs):
    try:
        ...
        emp_data = emp_data[0]  # Assuming the first result is the correct employee data
        
        # Extract and validate data
        data = kwargs
        payable_account = get_payable_account(emp_data.get("company"))
        
        # Convert amount to float (currency field)
        try:
            total_sanctioned_amount = float(data.get('amount', '0.0'))
            grand_total = float(data.get("amount", "0.0"))
        except ValueError:
            # Default value if conversion fails
            grand_total = 0.0
        
        frappe.msgprint(f"Calculated grand_total: {grand_total}")
        
        current_date = nowdate()  # This will get the current date in Frappe format
        expense_data = {
        ...
        # If editing an existing expense claim
        if data.get("name"):
            if not frappe.db.exists("Expense Claim", data.get("name")):
                return gen_response(500, "Invalid ID.")
            
            expense_doc = frappe.get_doc("Expense Claim", data.get("name"))
            if expense_doc.docstatus == 1:
                return gen_response(500, "Cannot modify a submitted document.")
            
            expense_doc.total_sanctioned_amount = total_sanctioned_amount
            expense_doc.grand_total = grand_total  # Set the grand_total (currency field)
            
            # Update the expense claim details
            expense_doc.employee = emp_data.name
            expense_doc.expense_approver = emp_data.expense_approver
            expense_doc.posting_date = current_date
            expense_doc.company = emp_data.get("company")
            expense_doc.payable_account = payable_account
            
            # Append new expense data to the existing list
            expense_doc.append('expenses', expense_data)
            expense_doc.save()  # Save the updated document
            
        else:    
            # For a new expense claim, assign the currency field
            expense_doc = frappe.get_doc({
                "doctype": "Expense Claim",
                "employee": emp_data.name,
                "expense_approver": emp_data.expense_approver,
                "posting_date": current_date,
                "grand_total": grand_total,  # Set to currency field
                "company": emp_data.get("company"),
                "payable_account": payable_account,
            })
            expense_doc.append('expenses', expense_data)
            expense_doc.insert()  # Insert the new document
        ...
        if data.get("attachments"):
            for file in data.get("attachments"):
                ...
                file_doc.attached_to_name = expense_doc.name
                file_doc.save()

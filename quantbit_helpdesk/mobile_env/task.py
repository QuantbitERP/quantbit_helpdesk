...
def create_task(**kwargs):
    try:
        ...
        # Update the task document with provided data
        task_doc.update({
            "subject": data.get("subject"),
            "project": data.get("project"),
            "priority": data.get("priority"),
            "parent_task": data.get("parent_task", ""),  # Optional field
            "status": data.get("status"),
            "description": data.get("description"),
            "exp_end_date": data.get("exp_end_date"),
            "expected_time": data.get("expected_time"),
            "actual_time": data.get("actual_time", 0),  # Optional field
            "progress": data.get("progress"),
            "issue": data.get("issue", ""),  # Optional field
        ...
        # Handle the assign_to functionality
        if data.get("assigned_to"):
            assign_to.add(
                {
                    "assign_to": data.get("assigned_to"),  # List of assignees
